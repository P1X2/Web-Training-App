...
function Plan() {
  ...
  const fetchPlan = () => {
    sendRequest("rest/plan/get", "GET",plan).then((data) => {
      setPlan(data);
    }).catch((error) => {
              ...
              <Row>
                ...
                {plan.exercises.map((exercise) => (
                  ...
                  <Card key={exercise.id} className="mb-3">
                    <Card.Body>
                      <Card.Title>{capitalizeFirstLetter(exercise.name)}</Card.Title>
                      <Card.Text>Reps: {exercise.reps}</Card.Text>
                      <Card.Text>Weight: {exercise.weight}</Card.Text>
                      <Card.Text> <a href={exercise.videoUrl} target="_blank" rel="noopener noreferrer">Tutorial Video</a></Card.Text>
